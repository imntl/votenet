...
from box_util import new_get_3d_box
...
def parse_predictions(end_points, config_dict):
    ...
    pred_heading_class = torch.argmax(end_points['heading_scores'], 2) # B,num_proposal,3
    pred_heading_residual = torch.gather(end_points['heading_residuals'], 2, pred_heading_class.unsqueeze(2)) # B,num_proposal,3
    ...
    for i in range(bsize):
        for j in range(num_proposal):
            ...
            corners_3d_upright_camera = new_get_3d_box(pred_center_upright_camera[i,j,:], box_size, heading_angle) 
...
def parse_groundtruths(end_points, config_dict):
    ...
    for i in range(bsize):
        for j in range(K2):
            ...
            corners_3d_upright_camera = new_get_3d_box(gt_center_upright_camera[i,j,:], box_size, heading_angle)
            gt_corners_3d_upright_camera[i,j] = np.array(corners_3d_upright_camera)
